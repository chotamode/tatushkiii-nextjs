...
const CATEGORY_KEYS = ['ornamental', 'lineWork', 'abstract', 'whipShading', 'freehand'] as const

// Decorative SVG overlays cycled across portfolio items (preserves the original
// per-item visual variety now that the grid is data-driven).
const DECOR_SVGS: ReactNode[] = [
  (
...
export default function PageClient() {
  ...
  const { portfolio, faqs, testimonials, services, settings } = useContent()
  ...
  const shownPortfolio = activeCategory
  ...
  return (
    <div className="antialiased overflow-x-hidden selection:bg-black selection:text-white">
      ...
      <div className="w-full bg-black text-white py-3 overflow-hidden border-y border-white/20">
        <div className="flex">
          <div className="flex whitespace-nowrap animate-marquee">
            {[0, 1, 2, 3].map((i) => (
              <span key={i} className="mx-4 font-mono text-xs tracking-widest uppercase flex items-center gap-4">
      ...
      <section id="portfolio" className="py-32 px-4 relative">
        ...
        <div className="container mx-auto">
            ...
            {CATEGORY_KEYS.map((key) => {
              ...
              return (
                <button
                  ...
                  className={`decoration-1 transition-colors relative group ${active ? 'text-black line-through' : 'text-gray-400 hover:text-black hover:line-through'}`}
                >
              ...
              >
                ...
                <div className="aspect-[3/4] bg-white relative overflow-hidden grayscale contrast-125 transition-all duration-700 ease-out group-hover:scale-[1.02]">
                  ...
                  <div className="absolute inset-0 flex items-center justify-center opacity-10 group-hover:opacity-30 transition-opacity">
                    {DECOR_SVGS[index % DECOR_SVGS.length]}
                  </div>
                ...
                <div className="mt-4 flex justify-between items-start border-t border-black pt-2 relative">
                  ...
                  <span className="font-mono text-[10px] opacity-50">{String(index + 1).padStart(3, '0')}</span>
                  {/* Small accent */}
          ...
          <div className="mt-32 text-center relative">
            ...
            <a href={settings.archive_url} target="_blank" rel="noopener noreferrer" className="group inline-flex flex-col items-center gap-2 relative">
            ...
            <div className="flex-1 w-full relative pt-12">
              <div className="aspect-[4/5] w-full border border-white/20 p-2 relative">
                ...
                <div className="w-full h-full bg-neutral-900 relative overflow-hidden grayscale contrast-150">
                  <Image src="/images/sandu.jpg" alt="Sandu - Tattoo Artist" fill sizes="(max-width: 768px) 100vw, 50vw" className="object-cover" />
                </div>
      ...
      <section id="process" className="py-32 px-4 bg-paper">
        <div className="container mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-12 max-w-6xl mx-auto">
            {faqs.map((faq, i) => {
              const lastOdd = faqs.length % 2 === 1 && i === faqs.length - 1
              return (
                <div
                  key={faq.id}
                  className={`group relative pt-6 border-t border-black/20 hover:border-black transition-colors duration-500 ${lastOdd ? 'md:col-span-2' : ''}`}
                >
                  <h3 className="font-display text-xl uppercase mb-4 group-hover:italic transition-all">{faq.question}</h3>
      ...
      {services.length > 0 && (
        <section id="prices" className="py-32 px-4 bg-paper border-t border-black/10">
          <div className="container mx-auto max-w-4xl">
            <div className="flex items-center justify-center gap-4 mb-20 opacity-50">
              <span className="sigil-text">⫘⫘⫘</span>
              <h2 className="font-mono text-sm uppercase tracking-[0.5em]">{t.prices.title}</h2>
              <span className="sigil-text">⫘⫘⫘</span>
            </div>

            <div className="flex flex-col divide-y divide-black/15">
              {services.map((service) => (
                <div key={service.id} className="flex items-baseline justify-between gap-6 py-6 group">
                  <div className="min-w-0">
                    <h3 className="font-display text-2xl uppercase group-hover:italic transition-all">{service.name}</h3>
                    {service.description && (
                      <p className="font-mono text-xs leading-relaxed text-gray-500 mt-1 max-w-md">{service.description}</p>
                    )}
                  </div>
                  <div className="font-mono text-sm whitespace-nowrap tracking-widest">{service.price}</div>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-16 md:gap-8 text-center md:text-left">
            {testimonials.map((item, i) => (
              <div key={item.id} className="flex flex-col gap-4">
                ...
                <p className={`font-display text-xl md:text-2xl leading-tight ${i % 2 === 1 ? 'text-gray-300' : ''}`}>
                  {item.text}
                </p>
                <div className="font-mono text-[10px] uppercase tracking-widest text-gray-500 mt-auto">
                  — {item.author} <span className="mx-2">/</span> {item.work}
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="container mx-auto max-w-4xl">
          ...
          <div className="mt-24 flex flex-col items-center gap-4 text-center">
            ...
            <div className="font-mono text-xs uppercase tracking-widest">
              <a href={settings.instagram_url} target="_blank" rel="noopener noreferrer" className="hover:line-through px-2">{t.contact.social.instagram}</a>
              <span className="opacity-30">/</span>
              <a href={settings.telegram_url} target="_blank" rel="noopener noreferrer" className="hover:line-through px-2">{t.contact.social.telegram}</a>
            </div>
            <div className="font-mono text-xs text-gray-400 mt-2">
              <a href={`tel:${settings.phone.replace(/\s/g, '')}`} className="hover:line-through">{settings.phone}</a>
              <span className="mx-2">•</span>
              <a href={`mailto:${settings.email}`} className="hover:line-through">{settings.email}</a>
            </div>
